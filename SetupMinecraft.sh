...
#!/bin/bash
# Minecraft Server Installation Script - Aesycos - https://aesycos.com
#
# Instructions: https://aesycos.com/minecraft-bedrock-edition-ubuntu-dedicated-server-guide/
# Resource Pack Guide: https://aesycos.com/minecraft-bedrock-server-resource-pack-guide/
#
...
#
# GitHub Repository: https://github.com/Aesycos/MinecraftBedrockServer

echo "Minecraft Bedrock Server installation script by Aesycos adapted from James A. Chambers"
echo "Latest version always at https://github.com/Aesycos/MinecraftBedrockServer"
...
Update_Scripts() {
  ...
  curl -H "Accept-Encoding: identity" -L -o start.sh https://raw.githubusercontent.com/Aesycos/MinecraftBedrockServer/master/start.sh
  chmod +x start.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o stop.sh https://raw.githubusercontent.com/Aesycos/MinecraftBedrockServer/master/stop.sh
  chmod +x stop.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o restart.sh https://raw.githubusercontent.com/Aesycos/MinecraftBedrockServer/master/restart.sh
  chmod +x restart.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o fixpermissions.sh https://raw.githubusercontent.com/Aesycos/MinecraftBedrockServer/master/fixpermissions.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o revert.sh https://raw.githubusercontent.com/Aesycos/MinecraftBedrockServer/master/revert.sh
  chmod +x revert.sh
  ...
  curl -H "Accept-Encoding: identity" -L -o update.sh https://raw.githubusercontent.com/Aesycos/MinecraftBedrockServer/master/update.sh
  chmod +x update.sh
...
Update_Service() {
  ...
  sudo curl -H "Accept-Encoding: identity" -L -o /etc/systemd/system/$ServerName.service https://raw.githubusercontent.com/Aesycos/MinecraftBedrockServer/master/minecraftbe.service
  ...
  if [[ "$answer" != "${answer#[Yy]}" ]]; then
    ...
    read answer </dev/tty
    if [[ "$answer" != "${answer#[Yy]}" ]]; then
      croncmd="$DirName/minecraft/$ServerName/restart.sh 2>&1"
      cronjob="0 4 * * * $croncmd"
      (
...
Check_Architecture() {
  ...
  if [[ "$CPUArch" == *"aarch"* || "$CPUArch" == *"arm"* ]]; then
    ...
    curl -H "Accept-Encoding: identity" -L -o depends.zip https://raw.githubusercontent.com/Aesycos/MinecraftBedrockServer/master/depends.zip
    unzip depends.zip
    sudo mkdir /lib64
    # Create soft link ld-linux-x86-64.so.2 mapped to ld-2.31.so, ld-2.33.so, ld-2,35.so
    sudo rm -rf /lib64/ld-linux-x86-64.so.2
    sudo ln -s $DirName/minecraft/$ServerName/ld-2.31.so /lib64/ld-linux-x86-64.so.2
    sudo ln -s $DirName/minecraft/$ServerName/ld-2.33.so /lib64/ld-linux-x86-64.so.2
    sudo ln -s $DirName/minecraft/$ServerName/ld-2.35.so /lib64/ld-linux-x86-64.so.2
  fi
...
Update_Sudoers() {
  if [ -d /etc/sudoers.d ]; then
    sudoline="$UserName ALL=(ALL) NOPASSWD: /bin/bash $DirName/minecraft/$ServerName/fixpermissions.sh -a, /bin/systemctl start $ServerName, /bin/bash $DirName/minecraft/$ServerName/start.sh"
...
until [ -d "$DirName" ]; do
  ...
  if [ ! -d "$DirName" ]; then
    echo "Invalid directory.  Please use the default path of /var/www or you're going to have errors.  This should be the same for ALL servers as it is your ROOT install directory."
  fi
done
...
if [ ! -d "minecraft" ]; then
  ...
  if [ -f "bedrock_server" ]; then
    echo "Migrating old Bedrock server to minecraft/old"
    cd $DirName
    mv minecraft old
    mkdir minecraft
    mv old minecraft/old
    cd minecraft
    echo "Migration complete to minecraft/old"
  fi
fi
...
if [[ "$ServerName" == *"minecraft"* ]]; then
  echo "Server label of minecraft is not allowed.  Please choose a different server label!"
  exit 1
fi
...
if [ -d "$ServerName" ]; then
  echo "Directory minecraft/$ServerName already exists!  Updating scripts and configuring service ..."
  ...
  cd minecraft
  cd $ServerName
  echo "Server directory is: $DirName/minecraft/$ServerName"
...
echo "Creating minecraft server directory ($DirName/minecraft/$ServerName)..."
cd $DirName
cd minecraft
mkdir $ServerName
cd $ServerName
